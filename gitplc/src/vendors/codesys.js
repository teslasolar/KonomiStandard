// CODESYS format adapter (IEC 61131-3 compliant)

/**
 * Type mapping from universal to CODESYS
 */
const TYPE_MAP = {
  'BOOL': 'BOOL',
  'INT': 'INT',
...
/**
 * Convert universal UDT to CODESYS DUT (Data Unit Type)
 */
export function toCodesys(udt) {
...
/**
 * Format default value for CODESYS
 */
...
/**
 * Parse CODESYS DUT to universal UDT
 */
export function fromCodesys(dutContent) {
  const nameMatch = dutContent.match(/TYPE (\w+) :/);
  if (!nameMatch) {
    throw new Error('Invalid CODESYS format: No TYPE declaration found');
  }
  ...
  const members = [];
  const memberRegex = /(\w+)\s*:\s*([^;:]+)(?::=\s*([^;]+))?;(?:\s*\(\*\s*(.+?)\s*\*\))?/g;
  ...
  while ((memberMatch = memberRegex.exec(dutContent)) !== null) {
    ...
    for (const [univ, codesys] of Object.entries(TYPE_MAP)) {
      if (dataType.trim() === codesys) {
        universalType = univ;
        break;
      }
    }

    // Arrays are already in universal format in CODESYS
...
/**
 * Generate CODESYS ST for PackML state machine
 */
